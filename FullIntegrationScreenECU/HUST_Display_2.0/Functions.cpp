...
void Write_Text_Color(Gpu_Hal_Context_t *phost, int x, int y, int size, char text[], int r, int g, int b)
{
  ...
  for(int i = 0; text[i] != '\0'; i++)
  {
    ...
    App_WrDl_Buffer(phost, VERTEX2II(x, y, 60, text[i]));
...
void draw_half_light(Gpu_Hal_Context_t *phost, int x1, int y1, int x2, int y2, int r, int g, int b) {
  App_WrDl_Buffer(phost, BEGIN(RECTS));
  App_WrDl_Buffer(phost, COLOR_RGB(r, g, b));
  App_WrDl_Buffer(phost, LINE_WIDTH(100));
  
  /* Using * 16 because the input to VERTEX2F is 1/16 pixel */    
  App_WrDl_Buffer(phost, VERTEX2F(x1 * 16, y1 * 16));
  App_WrDl_Buffer(phost, VERTEX2F(x2 * 16, (y1 + (y2 - y1) / 2) * 16));  // Adjust the y2 coordinate
